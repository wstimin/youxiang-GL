...
test('public page keeps mail and TOTP in separate tabs and forms', () => {
  ...
  assert.match(html, /query\.js\?v=20260820-1/);
...
test('public mail lookup supports bounded batches and automatic refresh', () => {
  ...
  assert.match(html, /id="single-code-result"/);
  ...
  assert.match(html, /id="mail-batch-toolbar" class="batch-result-toolbar hidden"/);
  assert.match(html, />接收结果<\/span>/);
  assert.doesNotMatch(html, /全部展开/);
  assert.match(script, /mailBatchToolbar\.innerHTML = `[\s\S]*?id="refresh-mail-batch"[\s\S]*?>自动刷新<\/span>/);
  assert.match(script, /mailBatchResultBox\.innerHTML = `<div class="batch-result-scroll"/);
  ...
  assert.match(script, /is-auto-refreshing/);
  assert.match(styles, /\.batch-result-toolbar \{[\s\S]*?display: flex/);
  assert.match(styles, /\.batch-summary-stats \{[\s\S]*?display: flex/);
  assert.match(styles, /\.batch-refresh-button\.is-auto-refreshing \.icon \{[\s\S]*?batch-refresh-spin/);
