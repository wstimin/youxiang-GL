...
这是一个可直接部署到 Linux 服务器的私人邮件与验证码查询系统。公开前台允许多人访问，但每个人必须使用管理员分配的独立查询密钥。管理员可以全局启用“验证码方式”，让单个查询只返回最新有效验证码；关闭后，单个查询可读取该密钥绑定子邮箱最近 7 天内的邮件。管理后台、邮箱凭据、子邮箱配置、未匹配邮件和审计记录仅管理员可访问。
...
- 全局单个查询方式开关：开启时返回最新有效验证码，关闭时返回最近 7 天文本邮件；批量查询始终只返回验证码
...
4. 将该密钥单独交给对应查询者。
5. 查询者访问公开首页，只输入密钥即可按管理员设置查看对应子邮箱的最新有效验证码或最近 7 天内的全部已归属邮件；批量模式始终用于查看每个子邮箱的最新有效验证码。
...
匹配成功的邮件会保存发件人、主题、接收时间和纯文本正文，正文使用 `MASTER_KEY_HEX` 进行 AES-256-GCM 加密并在 7 天后自动删除。系统不保存附件，不向浏览器返回邮件 HTML，也不会加载邮件中的远程图片。升级后 Worker 会为每个母邮箱执行一次最近 7 天邮件回补；仍保留在 IMAP 收件箱中的邮件可以补齐正文，已经从服务器删除的历史邮件无法恢复。
...
后台“运行概览”会显示邮件 Worker 心跳和当日查询指标；“安全设置”可以切换全局“验证码方式”、查看当前管理端登录会话，并退出其他浏览器或设备。切换查询方式不会重置子邮箱密钥，也不会删除已保存的邮件记录。
...
- “验证码方式”默认开启；关闭后仅单个查询可读取 7 天文本邮件，批量查询仍固定只返回最新有效验证码。
